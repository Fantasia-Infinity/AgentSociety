...
export function apply(ctx, config) {
    ...
    if (!hubUrl || !hubToken) {
        ctx.logger.warn('agent-society-question-bridge: Hub credentials are required; question bridge stays idle');
        return;
    }
    ...
    const policy = config.policy ??
        process.env.AGENT_SOCIETY_QUESTION_POLICY ??
        'auto';
    ...
    async function poll() {
        if (state.busy)
            return;
        if (policy === 'ask')
            return; // the human-facing card handles everything
        ...
        try {
            ...
            for (const question of questions) {
                ...
                try {
                    ...
                    ctx.logger.info(`agent-society-question-bridge answered ${questionId}`);
                }
                catch (error) {
                    ctx.logger.warn(`agent-society-question-bridge could not answer ${questionId}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
        catch (error) {
            ctx.logger.warn(`agent-society-question-bridge poll failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
            state.busy = false;
        }
...
//# sourceMappingURL=question-bridge.js.map