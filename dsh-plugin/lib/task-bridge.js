...
export function apply(ctx, config) {
    ...
    if (!hubUrl || !hubToken) {
        ctx.logger.warn('agent-society-task-bridge: Hub credentials are required; task bridge stays idle');
        return;
    }
    ...
    ctx.logger.warn(`agent-society-task-bridge active (hub=${hubUrl}, poll=${pollSeconds}s, actor=${actorId})`);
    ...
    const heartbeat = ctx.setInterval(() => {
        void hub.heartbeat(nodeId).catch((error) => {
            ctx.logger.warn(`agent-society-task-bridge heartbeat failed: ${error instanceof Error ? error.message : String(error)}`);
        });
    ...
    function debugDump(tag) {
        try {
            ...
            writeFileSync('/tmp/task-bridge-debug.json', `${JSON.stringify({ ts: Date.now(), tag, agentsVisible: agents !== undefined, rows }, null, 2)}\n`);
        }
        catch {
    ...
    async function poll() {
        ...
        try {
            ...
            ctx.logger.warn(`agent-society-task-bridge failed: ${error instanceof Error ? error.message : String(error)}`);
    ...
    async function execute(agent, task, run, leaseToken) {
        ...
        ctx.logger.info(`agent-society-task-bridge executing ${task.task_id} in session ${sessionId}`);
        ...
        ctx.logger.info(`agent-society-task-bridge completed ${task.task_id} in session ${sessionId}`);
