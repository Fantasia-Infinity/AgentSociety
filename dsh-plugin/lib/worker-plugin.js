...
export async function apply(ctx, config) {
    ...
    if (!hubUrl || !hubToken) {
        ctx.logger.warn('agent-society-worker: AGENT_SOCIETY_HUB_URL and AGENT_SOCIETY_HUB_TOKEN are required; worker stays idle');
        return;
    }
    const toolPolicy = normalizeToolPolicy(config.toolPolicy ?? process.env.AGENT_SOCIETY_TOOL_POLICY) ?? 'full';
    ...
    const worker = new WorkerLoop(ctx, hub, {
        ...
        statePath: resolve(process.env.DSH_HOME ?? `${process.env.HOME ?? '.'}/.dsh`, 'agent-society-worker-sessions.json'),
    });
...
class WorkerLoop {
                ...
                ...(process.env.AGENT_HUB_DSH_WEB === '1'
                    ? {
                        dsh_web: {
                            enabled: true,
                            protocol_version: '1',
                            profile: process.env.AGENT_SOCIETY_WEB_PROFILE ?? 'agent-society-web',
    ...
    async tick() {
        ...
        try {
            ...
            this.ctx.logger.warn(`agent-society-worker tick failed: ${message(error)}`);
        }
        finally {
            this.busy = false;
        }
    ...
    async claimControls(running) {
        try {
            ...
            for (const control of controls) {
                ...
                await this.hub.acknowledgeTaskControl(running.taskId, control.control_id, {
                    run_id: running.runId,
                    lease_token: control.lease_token,
                });
                this.ctx.logger.info(`agent-society-worker applied ${control.kind} control ${control.control_id}`);
            }
        }
        catch (error) {
    ...
    async execute(claim) {
            ...
            if (active === undefined) {
                ...
                if (previous) {
                    try {
                        ...
                        reused = true;
                        this.ctx.logger.info(`agent-society-worker resumed session ${previous} (${toolPolicy})`);
                    }
                    catch (error) {
                        this.ctx.logger.warn(`agent-society-worker resume failed, creating a new session: ${message(error)}`);
                    }
                }
            }
        }
        ...
        try {
            ...
            if (running.cancelled) {
                this.ctx.logger.info(`agent-society-worker cancelled ${task.task_id} in ${active.sessionId}`);
                return;
            }
            const text = lastAssistantText(agent.session.events);
            const finalTitle = this.currentSessionTitle(agent.session) ?? running.title;
            const transcript = await this.attachTranscript(task, run.run_id, agent.session, finalTitle, toolPolicy);
            await this.hub.updateTask(task.task_id, {
                run_id: run.run_id,
                lease_token: claim.lease_token,
                status: 'completed',
            ...
            if (running.cancelled) {
                ...
                await this.flushSession(agent.session);
                const transcript = await this.attachTranscript(task, run.run_id, agent.session, running.title, toolPolicy);
                await this.failTask(claim, message(error), {
                    sessionId: active.sessionId,
                    toolPolicy,
    ...
    async flushSession(session) {
        try {
            ...
            this.ctx.logger.warn(`agent-society-worker session flush failed: ${message(error)}`);
        }
    }
    applySessionTitle(session, task) {
        ...
        try {
            ...
            this.ctx.logger.warn(`agent-society-worker session title failed: ${message(error)}`);
    ...
    async attachTranscript(task, runId, session, title, toolPolicy) {
        try {
            ...
            if (raw) {
                ...
                if (bytes > 0 && bytes <= MAX_INLINE_ARTIFACT_BYTES) {
                    try {
                        ...
                        this.ctx.logger.warn(`agent-society-worker inline transcript artifact failed, attaching file reference: ${message(error)}`);
                    }
                }
            }
            return await this.hub.addArtifact({
    ...
    async executeSelfUpdate(claim) {
        ...
        try {
            ...
            this.ctx.logger.info(`agent-society-worker self-update ${report.needsRestart ? 'applied' : 'already up to date'}`);
    ...
    async markRunCancelled(running) {
        try {
            ...
            this.ctx.logger.warn(`agent-society-worker could not mark run ${running.runId} cancelled: ${message(error)}`);
        }
    }
    async failTask(claim, messageText, details = {}) {
        const { task, run } = claim;
        const result = {
            adapter: 'agent-society-dsh-plugin',
        ...
        try {
            ...
            this.ctx.logger.warn(`agent-society-worker could not report failure for ${task.task_id}: ${message(error)}`);
        }
    }
    taskWorkspace(task) {
...
function applySandboxMode(ctx, session, toolPolicy) {
    const mode = toolPolicy === 'full' ? 'workspace-write' : 'read-only';
    const sandboxPolicy = ctx.get('sandboxPolicy');
    if (sandboxPolicy?.overrideOf(session) === mode)
        return;
    setSandboxMode(session, mode);
}
function agentSetup(toolPolicy) {
    return (agentCtx) => {
        applyToolPolicy(agentCtx, toolPolicy);
    };
}
function applyToolPolicy(agentCtx, toolPolicy) {
    if (toolPolicy === 'full')
        return;
    ...
    const keep = names.filter((name) => allowed.has(name) || !LOCAL_TOOL_NAMES.has(name));
    if (keep.length > 0) {
...
function taskToolPolicy(task, fallback) {
...
function taskTitle(task) {
    ...
    if (!objective)
        return `Hub task ${task.task_id.slice(0, 8)}`;
    return truncateTitleUtf8(objective);
}
function truncateTitleUtf8(value, maxBytes = 80) {
