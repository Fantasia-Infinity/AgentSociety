...
export function apply(ctx, config) {
    ...
    if (!hubUrl || !hubToken) {
        ctx.logger.warn('agent-society-directory: AGENT_SOCIETY_HUB_URL and AGENT_SOCIETY_HUB_TOKEN are required; directory sync stays idle');
        return;
    }
    ...
    async function sync() {
        try {
            ...
            ctx.logger.warn(`agent-society-directory sync failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async function pullConsensus(full) {
