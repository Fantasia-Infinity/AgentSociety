...
class HubRequestHandler(WebHandlersMixin, BaseHTTPRequestHandler):
    ...
    def _web_proxy(self, method: str) -> None:
        ...
        if len(segments) < 3 or segments[0] != "v1" or segments[1] != "web":
            ...
            return
        node_id = segments[2]
        # Slice the original path instead of rejoining split segments: the
        # device surface uses paths such as `/plugins/` with a query-only
        # plugin id (`/plugins/??pkg&rev=N`), and rejoining drops the trailing
        # slash, which then fails the allowlist check below.
        mount = f"/v1/web/{node_id}"
        if rest.startswith(mount):
            path = rest[len(mount) :] or "/"
        elif len(segments) > 3:
