...
export class WebBridge {
  ...
  private async handleMessage(ws: WebSocket, data: unknown): Promise<void> {
    ...
    const relayedHeaders = Object.fromEntries(response.headers.entries());
    // Node's fetch returns the decompressed body while keeping the original
    // encoding/framing headers. Dropping them keeps the relayed body honest;
    // the Hub sets its own Content-Length afterwards.
    delete relayedHeaders["content-encoding"];
    delete relayedHeaders["content-length"];
  ...
  private openEventStream(ws: WebSocket, message: Record<string, unknown>): void {
    ...
    let local: WebSocket;
    try {
      // Node/undici's WebSocket reads the init object (headers) from the
      // SECOND argument; the third-position options form is ignored, which
      // silently dropped the local session cookie and made every event
      // stream fail its upgrade.
